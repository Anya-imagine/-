...
class Session(BaseSession):
    ...
    def get_readable_fields(self):
        ...
        result.update({
            ...
            "src_port": int(self.port1) if self.port1 else 0,
            "dst_port": int(self.port2) if self.port2 else 0,
            "ip_protocol": int(self.ip_protocol) if self.ip_protocol is not None else 0,
            "is_ipv6": bool(self.is_session_v6),
        ...
        # 根据协议类型添加特定字段
        if self.protocols:
            protocols_list = [str(proto).lower() for proto in self.protocols]
            result["protocols"] = protocols_list
            
            # HTTP 相关字段
            if any(p in protocols_list for p in ["http", "https"]):
                result["headers"] = {
                    "request": {str(k): str(v) for k, v in self.headers['request'].items()} if self.headers.get('request') else {},
                    "response": {str(k): str(v) for k, v in self.headers['response'].items()} if self.headers.get('response') else {}
                }
                result["cookies"] = {str(k): str(v) for k, v in self.cookies.items()} if self.cookies else {}
                result["params"] = {str(k): str(v) for k, v in self.params.items()} if self.params else {}
            
            # DNS 相关字段
            if "dns" in protocols_list:
                if hasattr(self, 'dns_query_type'):
                    result["dns_query_type"] = self.dns_query_type
                if hasattr(self, 'dns_query_name'):
                    result["dns_query_name"] = self.dns_query_name
                if hasattr(self, 'dns_response_code'):
                    result["dns_response_code"] = self.dns_response_code
            
            # TLS 相关字段
            if "tls" in protocols_list:
                if hasattr(self, 'tls_version'):
                    result["tls_version"] = self.tls_version
                if hasattr(self, 'tls_cipher'):
                    result["tls_cipher"] = self.tls_cipher
                if hasattr(self, 'tls_ja3'):
                    result["tls_ja3"] = self.tls_ja3
            
            # SSH 相关字段
            if "ssh" in protocols_list:
                if hasattr(self, 'ssh_version'):
                    result["ssh_version"] = self.ssh_version
                if hasattr(self, 'ssh_key_exchange'):
                    result["ssh_key_exchange"] = self.ssh_key_exchange
            
            # SMB 相关字段
            if "smb" in protocols_list:
                if hasattr(self, 'smb_command'):
                    result["smb_command"] = self.smb_command
                if hasattr(self, 'smb_share'):
                    result["smb_share"] = self.smb_share
                if hasattr(self, 'smb_file'):
                    result["smb_file"] = self.smb_file
        
        # 添加标签（如果有）
        if self.tags:
            result["tags"] = [str(tag) for tag in self.tags]
